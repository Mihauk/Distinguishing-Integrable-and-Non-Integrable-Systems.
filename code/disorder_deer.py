...
a = np.array([0.5,1,1.5,2,3,3.5,4,5,6,7,15,20])
D = np.zeros((dis_sam, samples, a.size, Ngrid))
	...
	for k in range(dis_sam):
		...
		for j in range(samples):
			...
			for i in range(Ngrid):
				...
				D[k, j, l, i] = cm.check_real(np.dot(chi_t_dagg, np.dot(sigma_z_1,chi_t)))

with open('/home/abhishek/Documents/ny project/data/hv5_N'+str(N)+'_s3_d2_rc_disorder_tav_tsi.pickle', 'wb') as data: #two spin interaction
